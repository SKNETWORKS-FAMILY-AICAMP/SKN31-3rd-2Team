...
ROUTE_PROMPT = ROUTE_PROMPT = """당신은 사용자 발화가 '군 법률·규정 참조 문서 검색'이 필요한지 판단하는 분류기입니다.
이 챗봇은 군 생활 법률·규정 상담이 목적이므로, 대부분의 발화는 검색이 필요합니다.

"yes" (아래 중 하나라도 해당):
- 군 법률·규정·훈령·권리·의무·처벌·휴가·징계·복무·급여·복지·수당·전역·상담 관련 질문
- 사용자가 겪은 상황·불만·고충·피해를 서술하는 발화
- 무엇을 할 수 있는지/해도 되는지/부당한지/규정 위반인지 묻는 발화
- 절차·방법·금액·연락처 등 실무 정보를 묻는 발화

"no" (아래에만 해당):
- 순수한 인사말 ("안녕", "고마워")
- 챗봇 자기소개 요청 ("너 누구야?", "뭐 할 수 있어?")
- 군 생활과 전혀 무관한 잡담

판단이 조금이라도 애매하면 "yes". 상황을 서술하거나 감정을 토로하는 발화는 거의 항상 "yes"입니다.

예시:
Q: "휴가를 통제당했어요. 어떻게 하나요?" → yes
Q: "선임이 사적으로 심부름 시키는데 거부해도 되나요?" → yes
Q: "이거 억울한데 신고할 수 있어요?" → yes
Q: "안녕! 넌 누구야?" → no
Q: "오늘 날씨 어때?" → no
...
SOURCE_PROMPT = """당신은 군인 지원 시스템의 질문 분석기(Router)입니다. 
사용자 질문을 분석하여 'GUIDE'(길라잡이 Qdrant)와 'LAW'(법령 Neo4j) 중 어떤 DB를 조회해야 하는지 판단하고 반드시 지정된 JSON 포맷으로만 답변하세요.

[분류 기준]
1. GUIDE (길라잡이 Qdrant 대상):
   - 병영 복지, 생활 편의, 자기개발, 사회적응 지원, 진료 신청 절차, 인사제도/근무 안내 등 '가이드북' 성격의 부드러운 일상 질문.
   - 예: "휴가 갈 때 기차 할인 어떻게 받나요?", "군대에서 자격증 공부하는 법", "전역 후 사회 적응 프로그램 추천"

2. LAW (법령 Neo4j 대상):
   - 법률, 시행령, 훈령, 징계, 보수/수당, 재해보상법 등 명확한 '조문/규정/처벌/법적 권리'를 확인해야 하는 질문.
   - 특히 사진 속 주요 법령(군인사법, 군형법, 병역법, 군인보수법, 군인 징계령, 부대관리훈령 등)과 직접적으로 관련된 처벌, 의무, 수당 액수 기준, 법적 절차 질문.
   - 예: "지시 불이행 시 징계 수위", "초임호봉 획정 법적 기준", "군형법상 무단이탈 처벌", "군인재해보상법상 보상금 기준"
...
class LangGraphChatbot:
    ...
    def check_reference_node(self, state: ChatBotState) -> Dict[str, Any]:
        ...
        needs_reference = not verdict.strip().lower().startswith("no")

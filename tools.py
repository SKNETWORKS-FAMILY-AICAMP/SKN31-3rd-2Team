...
def search_guidance_knowledge_base(user_query: str) -> str:
    """군 생활 '가이드북성 실무 안내'를 검색합니다. 병영 복지, 생활 편의, 행정 절차 등
        ...
        (예: "휴가 신청 방법"은 이 도구, "정당한 사유 없이 휴가를 취소당했는데 규정 위반인지"는 search_law_knowledge_graph)
...
def search_law_knowledge_graph(user_query: str) -> str:
    ...
    다음과 같이 명확한 법적 처벌/징계/권리·의무의 근거가 필요한 질문에 사용하세요:
    - 무단이탈, 지시 불이행, 명령 불복종 등에 대한 처벌 수위·징계 종류
    - 특정 조 번호(예: '제8조')나 법령명이 직접 언급된 질문
    - 초임호봉 획정, 보수/수당 지급 기준 등 '법적 근거'가 필요한 질문
    - 지휘관의 조치(휴가 취소, 인사 조치 등)가 '규정 위반인지 아닌지' 판단이 필요한 질문
    - 서로 다른 법령·훈령·시행령 간의 관계나 상하위 규정 확인
 
    사용하지 않는 경우: 복지/생활 편의, 신청 절차, "어떻게 하면 되나요" 류의
    실무 안내 질문은 대신 search_guidance_knowledge_base를 사용하세요.
    (예: "휴가 신청 방법"은 search_guidance_knowledge_base, "정당한 사유 없이 휴가를 취소당했는데 규정 위반인지"는 이 도구)
